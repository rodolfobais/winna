	...
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			*/	
			...
			$mmpeximlist = array();
			if ($this->user->hasPermission('access', 'tool/mmexport')) {		
				$mmpeximlist[] = array(
					'name'	   => $this->language->get('text_pexim'),
			...
			if ($mmpeximlist) {					
				$data['menus'][] = array(
					'id'       => 'mm-productimportexport',
					'icon'	   => 'fa fa-external-link', 
					'name'	   => $this->language->get('text_mmpexim_list'),
					'href'     => '',
					'children' => $mmpeximlist
				);		
			}
