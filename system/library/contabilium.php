...
class Contabilium
{
    ...
    public function token($store_id = 0)
    {
        ...
        $tokencache = unserialize($this->cache->get('module_contabilium_token_cache' . $store_id));
        ...
        if ($maketoken && !empty($user) && !empty($key)) {
            $data = $this->post($this->api_base . 'token', array('client_id' => $user, 'client_secret' => $key, 'grant_type' => 'client_credentials'), $store_id, true);
            ...
            if ($data !== false && !is_null($data) && !isset($data->error)) {
                ...
                $this->cache->set('module_contabilium_token_cache' . $store_id, serialize($cache));
     ...
     */
    public function get($url, $store_id = 0)
    {
        $ch = curl_init();

        $token = $this->token($store_id)->access_token;
     ...
     */
    public function post($url, $params, $store_id = 0, $isAuthRequest = false, $debug = false)
    {
        $ch = curl_init();
