		...
		foreach( $this->labels as $label ) {
			...
			if ( $label['status'] && $this->ShowOnthisLayout($label,$is_product_layout,$is_home) && $layout_position_ok && !$this->ExpiredLabel($label) && $show_on_this_category) {
				...
				if ( !empty($label['offsetx']) || $label['offsetx'] == '0' ) {
					if ($label['style'] == 'rotated') {
						$css_cut_rotated .= 'left:'.$label['offsetx'].'%;';
					} else {
				...
				if ( !empty($label['offsety']) || $label['offsety'] == '0' ) {
					if ($label['style'] == 'rotated') {
						$css_cut_rotated .= 'top:'.$label['offsety'].'%;';
					} else {
				...
				if ( !empty($label['custom_css']) ) {
					$custom_css = explode('|', $label['custom_css']);
					if (isset($custom_css[0])) {
						if (!empty($custom_css[0])) {
							$css .= $custom_css[0];
						}
					}
				}
					...
					case 'regex':
						...
						if ( $regex_pattern ) {
							...
							if ( preg_match($regex_pattern, $property_value, $matched) ) {
								...
								if ( !empty($subtitle) ) {
									if ( stripos($subtitle,'[matched]')!==false ) {
				...
				if ( !empty($subtitle) ) {
						...
						$custom_css_subtitle = isset($custom_css[1])?$custom_css[1]:"";
						$subtitle = '<span style="'.$custom_css_subtitle.'position:absolute;left:'.$subtitlex.'%;top:'.$subtitley.'%;padding:0;margin:0;width:100%;font-size:'.$subtitlesize.$subtitlecolor.';">'.$subtitle.'</span>';
				} 
				...
				if ( $add_label ) {
					...
					$div_cut_rotated = '<div class="cut_rotated">';
					if ($css_cut_rotated) {
						$div_cut_rotated = '<div class="cut_rotated" style="'.$css_cut_rotated.'">';
					}

					$label_str .= $label['style']=='rotated' ? $div_cut_rotated : ''; // comment if any issue with mouseover on images
