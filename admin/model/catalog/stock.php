<?php
class ModelCatalogStock extends Model
{
	...
	public function getDuplicateCombinationNames($combinations, $translations = TRUE, $firstOnly = TRUE, $separator = '-')
	{
		...
		foreach ($combinations as $combination) {
			...
			$key = implode('-', $option_values) . '-' . $combination["store_id"];
	...
	public function getCombinations($data = array())
	{
		...
		$sql = "SELECT s.product_id, p.model, p.quantity as product_quantity, s.combination_id, s.quantity, s.store_id, s.sku, pd.name as product_name, so.product_option_value_id, ovd.name as option_value_name, pov.product_option_id, od.name as option_name "
			. "FROM `" . DB_PREFIX . "stock` s "
		...
		if (isset($data['product_combinations'])) {
			foreach ($data['product_combinations'] as $combination) {
				...
				if ($quantity >= 0) {
					...
					$this->db->query("INSERT INTO `" . DB_PREFIX . "stock` SET combination_id = '" . (int) $combination_id . "', product_id = '" . (int) $product_id . "', quantity = '" . (int) $quantity . "', store_id = '" . (int) $combination['store_id'] . "', sku = '" . $combination['sku'] . "'");
					$added = TRUE;
