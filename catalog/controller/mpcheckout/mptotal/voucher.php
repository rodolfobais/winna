<?php
class ControllerMpcheckoutMptotalVoucher extends Controller {
	public function index() {
		if ($this->config->get('voucher_status')) {
			$this->load->language('mpcheckout/mptotal/voucher');
			...
			if(VERSION < '2.2.0.0') {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mpcheckout/mptotal/voucher.tpl')) {
			    	return $this->load->view($this->config->get('config_template') . '/template/mpcheckout/mptotal/voucher.tpl', $data);
			   } else {
			   		return $this->load->view('default/template/mpcheckout/mptotal/voucher.tpl', $data);
			   }
		  	} else{
			   return $this->load->view('mpcheckout/mptotal/voucher', $data);
			}
		}
	}
