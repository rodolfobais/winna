...
class ControllerMpcheckoutMptotalReward extends Controller {
	public function index() {
		...
		if ($points && $points_total && $this->config->get('reward_status')) {
			...
			if(VERSION < '2.2.0.0') {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mpcheckout/mptotal/reward.tpl')) {
			    	return $this->load->view($this->config->get('config_template') . '/template/mpcheckout/mptotal/reward.tpl', $data);
			   } else {
			   		return $this->load->view('default/template/mpcheckout/mptotal/reward.tpl', $data);
			   }
		  	} else{
