<div id="popup-<?php echo $module_id; ?>" class="popup <?php if($type == 1) { echo 'popup-newsletter '; } ?> <?php if(!$ajax) { echo 'mfp-hide'; } ?>" style="<?php if($content_width > 100) { echo 'max-width: ' . $content_width . 'px;'; } if($background_color != '') { echo 'background-color: ' . $background_color . ';'; } if($background_image != '') { echo 'background-image: url(image/' . $background_image . ');background-position: ' . $background_image_position . ';background-repeat: ' . $background_image_repeat . ';'; } ?>">
     <?php 
          if($type == 1) { ?>
               
               <div class="newsletter-content">
                    <div class="newsletter-inner clearfix">
                         <div class="text-center col-xs-12 col-sm-7 col-md-7 col-lg-7">
                              <div class="newsletter-title"><h3 class="text-center"><?php echo $newsletter_popup_title; ?></h3></div>
                              <div class="popup_message">
                                   <div class="intro"><?php echo $newsletter_popup_text; ?></div>
                                   <div class="input-box">
                                        <input name="email" type="text" id="typo-newsletter" value="<?php echo $newsletter_input_placeholder; ?>" onclick="this.value=='<?php echo $newsletter_input_placeholder; ?>'?this.value='':''" onblur="this.value==''?this.value='<?php echo $newsletter_input_placeholder; ?>':''" class="input-text required-entry validate-email email">
                                        <button type="submit" title="Subscribe" class="button subscribe"><span><span><?php echo $newsletter_subscribe_button_text; ?></span></span></button>
                                   </div>
                              </div>
                              
                              <?php if($display_text_dont_show_again == 0 && !$ajax) { ?>
                              <div class="not-show-popup">
                                   <input type="checkbox" class="dont-show-me" id="discheck">
                                   <label class="subcriber_label" for="discheck"><span><?php echo $text_dont_show_again; ?></span></label>
                              </div>
                              <?php } ?>
                         </div>
                    </div>
     ...
     <?php if($display_text_dont_show_again == 0 && !$ajax && $type != 1) { ?>
